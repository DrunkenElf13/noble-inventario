...
COLS_INSUMOS = [
    ...
    "Espacio_4",              # K
    "Stock Mínimo",           # L
    "Espacio_5",              # M — columna reservada
    "Espacio_6",              # N — columna reservada
    "Espacio_7",              # O — columna reservada
                                ...
                                if not e_n.strip():
                                    ...
                                    if err:
                                        ...
                                        try:
                                            ...
                                            ws_ins.update(
                                                range_name=f"A{idx}:Q{idx}", values=fila_act
                                            )
                                            st.cache_data.clear()
            ...
            for idx_row, row in df_f.iterrows():
                ...
                with c7:
                    ...
                    if prev is not None:
                        ...
                        v_comprar_prev = False
                    ck_key = f"p_{safe_nom}"
                    if ck_key not in st.session_state:
                        st.session_state[ck_key] = v_comprar_prev
                    v_p = st.checkbox("🛒", key=ck_key)
                with c8:
                    obs_key = f"c_{safe_nom}"
                    v_c = st.text_input(
                        "Obs", key=obs_key,
                        label_visibility="collapsed",
