...
# --- 4. SIDEBAR INTEGRAL (EQUIPO + CATÁLOGO COMPLETO) ---
with st.sidebar:
    ...
    st.divider()
    # Gestión del Catálogo (Completo con todos los campos)
        ...
        if not df_raw.empty and "Nombre del Insumo" in df_raw.columns:
            ...
            with st.form("f_edit"):
                ...
                if st.form_submit_button("💾 Actualizar Insumo"):
                    ...
                    # Actualiza toda la fila de la A a la L de una sola vez
...
if st.session_state.pagina == "Dashboard":
    ...
    if not df_historial.empty:
        try:
            # Filtra para dejar solo el último registro de cada insumo por sucursal
...
# --- 6. PÁGINA: INVENTARIO (CAPTURA DIGITAL) ---
    ...
    # Filtrar insumos según selección
    ...
    if not df_f.empty:
        ...
        if st.button("📥 SUBIR INVENTARIO A LA NUBE", use_container_width=True, type="primary"):
            ...
            for n, info in regs.items():
                dm = info["row"]
                # Estructura exacta para la hoja "Historial"
                filas.append([
                    u_sel, 
                    n, 
                    dm.get('Marca',''), 
            ...
            st.cache_data.clear()
            st.success("¡Inventario procesado y guardado con éxito!")
            st.balloons()

# --- 7. PÁGINAS DE TICKETS TÉRMICOS (58MM) ---
